...
Функция ПолучитьФайлНастроекПоУмолчанию() Экспорт
	...
	Иначе
		Возврат ОбъединитьПути(ПолучитьПеременнуюСреды(ВРег("USERPROFILE")), "appdata\roaming\1c\1cestart\ibases.v8i");
	КонецЕсли;
...
//
// Параметры:
//	ТолькоЭлементы - Булево - Вернуть только элементы, без групп. По умолчанию Ложь - вернет элементы и группы
//
// Возвращаемое значение:
//  Соответствие - Список баз, где: Ключ - имя базы, Значение: Структура с параметрами соединения
//
Функция ПолучитьСписокБаз(ТолькоЭлементы = Ложь) Экспорт
	...
	Для каждого База из Базы Цикл
		...
		Если (ТолькоЭлементы) И (НЕ СтруктураПути.Свойство("Connect")) Тогда
			Продолжить;
		КонецЕсли;
